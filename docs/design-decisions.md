...
## DD-008 今日のタスクカードは未完了だけを描き、正本と衝突する要素は作らない
...
完成モックの「11. 今日のタスク」を Dashboard へ移植するにあたり、次を確定する。

**1. 描くのは未完了タスクだけ**

モックは完了済み2件をカード上部に置いているが、正本10章は
「完了済み Task の配置位置は未確定」と明示している。
モックがそう描いているという理由で配置を確定させない。product-spec を優先する。

`Dashboard#today_completed` はデータを供給できる状態のまま、描画だけ保留する。

**2. `▾` は使わず、未完了は空のチェックとして描く**

モックでは未完了が `▾`、完了が `✓` で、JS はこの2つを入れ替えているだけである。

```js
el.textContent = t.classList.contains('done') ? '✓' : '▾';
```

`.chk` に展開先の要素もCSSも存在しないため、`▾` に「開く」意味はない。
そのまま移植すると、機能はないのに操作の意味だけが分かりにくくなる。

`.chk` のサイズ・位置・質感はモックどおり維持し、**中の記号だけを変える**。
未完了は空（枠のみ）、完了は `✓`。新しいアイコンを作らない。

**3. 「＋ 追加」はカード内のインラインフォームを開く**

正本に既にある「＋ 追加」を、DD-005 の `POST /tasks` へ繋ぐ最小のUIとして扱う。
展開後は title / due_on / 保存・キャンセル のみ。凝った設計はしない。

Stimulus は「フォームの開閉」だけを担当する。Task の作成そのものをJSに持たせない。
バリデーションに失敗したとき（422）は、開いた状態・入力値・エラーを保ったまま描画する。
この「開いた状態」はサーバが描画するものであり、JSの状態復元に依存しない。

**4. 正本と衝突する要素は作らない**

| モックの要素 | 作らない理由 |
| --- | --- |
| `.tag`（習慣 / 自己投資 / 学業 / 発信） | 正本2章「category は初回 Task テーブルに含めない」 |
| `.star` ★ | 正本2章「priority は含めない」 |
| `.exp`（10 / 15 / 30 / 20 EXP） | 正本9章「報酬額は Task ごとに変えない」。モックは可変EXP前提で、正本と矛盾する |
| `.mission`（今日のミッション🎁） | 別ドメイン。正本に記述がない |

カードの骨格・余白・区切り線はモックどおりにし、行の中身は正本が認めた要素だけにする。

**5. Componentは2つに分ける**

* `Dashboard::TodayTasksComponent` … カード全体（`.tasks.card`）
* `Dashboard::TaskComponent` … 1行（`.task`）

行は「完了/未完了の描き分け」と「completion への form」を持つため、カードへ畳むと
`each` の中に分岐とformが入り、before-analysis 5章の「Viewの肥大化」に向かう。
Componentはデータを取りに行かず、必要なものを引数で受け取る（DD-006）。

### 保留事項（このカードだけでは到達できない操作）

**完了済みTaskを描画しないため、画面から「完了取消」へ到達できない。**
`DELETE /tasks/:task_id/completion`（DD-005）は生きているが、押せるUIが無い。
同じ理由で `DELETE /tasks/:id`（削除）も、モックに対応する要素が無いため接続していない。

これはバグではなく、配置が未確定であることの帰結である。
**完了済みTaskの配置（上 / 下 / 折りたたみ / 別領域）を決めるときに、
completion の DELETE と Task の削除のUIも同時に接続すること。**
到達できないからという理由で、配置の決定を前倒ししない。

### 既知の制約

「＋ 追加」のフォーム展開は Stimulus によるため、JavaScript 無効時は未展開状態から
新規作成フォームへ到達できない。422時のエラー表示はサーバ側で開状態を描画するため表示可能。
現時点では DD-008 の要求範囲外とし、アクセシビリティ要件を見直す際の検討事項として保留する。

### 暫定 `<ul>` の撤去

DD-006 で置いた暫定の `<ul>` は、`Dashboard::TodayTasksComponent` を配置する
**同じ差分の中で削除する**。二重に描画される状態を1コミットも残さない。
撤去漏れを検出できるよう、同じタスクが本文へ2回現れないことを spec で確かめる。
